...
const {
  ...
  startRest,
  skipRest,
  workouts, // ADD THIS TO GET LATEST WORKOUT DATA
} = useWorkout()
...
// GET THE LATEST WORKOUT DATA - THIS IS THE FIX
const getLatestWorkoutData = () => {
  if (contextActiveWorkout && workouts) {
    // Find the latest version of the workout from the workouts array
    const latestWorkout = workouts.find(w => w.id === contextActiveWorkout.id)
    if (latestWorkout) {
      // Merge the latest workout structure with the active workout progress
      return {
        ...latestWorkout, // Use latest workout data (sets, reps, etc.)
        exercises: latestWorkout.exercises.map(ex => {
          // Find the corresponding exercise in the active workout to preserve progress
          const activeEx = contextActiveWorkout.exercises?.find(activeEx => activeEx.id === ex.id)
          if (activeEx) {
            return {
              ...ex, // Use latest exercise data
              completed: activeEx.completed || new Array(ex.sets).fill(ex.reps),
              setsCompleted: activeEx.setsCompleted || new Array(ex.sets).fill(false),
              lastPerformance: activeEx.lastPerformance,
              isCompleted: activeEx.isCompleted || false,
            }
          }
          // If no active exercise found, create new one
          return {
            ...ex,
  ...
  return contextActiveWorkout
}

const currentWorkout = getLatestWorkoutData() || contextActiveWorkout
...
useEffect(() => {
  ...
  if (contextActiveWorkout) {
    // Update the context with the latest workout data
    const latestWorkout = getLatestWorkoutData()
    if (latestWorkout && JSON.stringify(latestWorkout) !== JSON.stringify(contextActiveWorkout)) {
      updateActiveWorkout(latestWorkout)
    }
  } else if (workout) {
    setActiveWorkout(workout)
  }
}, [contextActiveWorkout, workout, workouts]) // ADD workouts as dependency
...
}, [currentWorkout, isWorkoutRunning])
